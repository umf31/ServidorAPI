...
{
    public class ExcepcionNoEncontrado : IExceptionFilter
    {
        public void OnException(ExceptionContext context)
        {
            if (context.Exception.GetType() == typeof(NotFound))
            {
                var excepcion = (NotFound)context.Exception;
                var validacion = new DetalleRespuesta
                {
                    Resultado = false,
                    Encabezado = Mensaje.Encabezado.StatusCode404,
                    Detalle = excepcion.Message,
                    StatusCode = Mensaje.Excepcion.StatusCode404,
