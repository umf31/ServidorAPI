...
{
    public class LogicaCaispn01Unidad<Indicador> : ILogicaCaispn01Unidad<Caispn01Unidad>
    {
        private readonly IMapper mapper;
        private readonly ISadimUT uts;
        private readonly IPagSadim lista;
        private readonly ICrearIndicador crear;
        public LogicaCaispn01Unidad(IMapper _mapper, ISadimUT _uts, IPagSadim _lista, ICrearIndicador _crear)
        {
            mapper = _mapper;
            uts = _uts;
            lista = _lista;
        ...
        public async Task<Lista<Caispn01Unidad>> ObtenerTodo(dynamic dynConsulta)
        {
            ...
            var entidad = await uts.AsistenteCaispn01Unidad.ObtenerTodoFiltros();
            if (consulta.Id != null)
            {
            ...
            var entidadPaginada = await lista.Caispn01Unidad.CrearLista(entidad, consulta.NumeroPagina, consulta.NumeroRegistros);
        ...
        public async Task<bool> Actualizar(string matricula)
        {
            ...
            var existe = await uts.AsistenteCaispn01Unidad.ExisteEntidadPorNombre(indicadorNombre);
            if (existe != false)
            {
                var ultimoRegistro = await uts.AsistenteCaispn01Unidad.ObtenerUltimoRegistro(indicadorNombre);
                var cp04 = await uts.AsistenteCaispn01Unidad.ObtenerPeriodosCP04(ultimoRegistro.Periodo);
                var crearIndicador = mapper.Map<List<Caispn01Unidad>>(cp04);
                var indicador = await crear.CrearCAISPN01(crearIndicador, matricula);

                await uts.AsistenteCaispn01Unidad.Eliminar(ultimoRegistro.Id);
                await uts.AsistenteCaispn01Unidad.InsertarLista(indicador);
            }
            else
            {
                var cp04 = await uts.AsistenteCaispn01Unidad.ObtenerPeriodosCP04(Indicadores.Periodo.Inicio);
                var crearIndicador = mapper.Map<List<Caispn01Unidad>>(cp04);
                var indicador = await crear.CrearCAISPN01(crearIndicador, matricula);
                await uts.AsistenteCaispn01Unidad.InsertarLista(indicador);
            }
