...
{
    public class LogicaCaispn02Unidad<Indicador> : ILogicaCaispn02Unidad<Caispn02Unidad>
    {
        private readonly IMapper mapper;
        private readonly ISadimUT uts;
        private readonly IPagSadim lista;
        private readonly ICrearIndicador crear;
        public LogicaCaispn02Unidad(IMapper _mapper, ISadimUT _uts, IPagSadim _lista, ICrearIndicador _crear)
        {
            mapper = _mapper;
            uts = _uts;
            lista = _lista;
        ...
        public async Task<Lista<Caispn02Unidad>> ObtenerTodo(dynamic dynConsulta)
        {
            ...
            var entidad = await uts.AsistenteCaispn02Unidad.ObtenerTodoFiltros();
            if (consulta.Id != null)
            {
            ...
            var entidadPaginada = await lista.Caispn02Unidad.CrearLista(entidad, consulta.NumeroPagina, consulta.NumeroRegistros);
        ...
        public async Task<bool> Actualizar(string matricula)
        {
            ...
            var existe = await uts.AsistenteCaispn02Unidad.ExisteEntidadPorNombre(indicadorNombre);
            if (existe != false)
            {
                var ultimoRegistro = await uts.AsistenteCaispn02Unidad.ObtenerUltimoRegistro(indicadorNombre);
                var iap01 = await uts.AsistenteCaispn02Unidad.ObtenerPeriodosCP04(ultimoRegistro.Periodo);
                var crearIndicador = mapper.Map<List<Caispn02Unidad>>(iap01);
                var indicador = await crear.CrearCAISPN02(crearIndicador, matricula);

                await uts.AsistenteCaispn02Unidad.Eliminar(ultimoRegistro.Id);
                await uts.AsistenteCaispn02Unidad.InsertarLista(indicador);
            }
            else
            {
                var iap01 = await uts.AsistenteCaispn02Unidad.ObtenerPeriodosCP04(Indicadores.Periodo.Inicio);
                var crearIndicador = mapper.Map<List<Caispn02Unidad>>(iap01);
                var indicador = await crear.CrearCAISPN02(crearIndicador, matricula);
                await uts.AsistenteCaispn02Unidad.InsertarLista(indicador);
            }
