...
{
    public class LogicaCaispn09Unidad<Indicador> : ILogicaCaispn09Unidad<Caispn09Unidad>
    {
        private readonly IMapper mapper;
        private readonly ISadimUT uts;
        private readonly IPagSadim lista;
        private readonly ICrearIndicador crear;
        public LogicaCaispn09Unidad(IMapper _mapper, ISadimUT _uts, IPagSadim _lista, ICrearIndicador _crear)
        {
            mapper = _mapper;
            uts = _uts;
            lista = _lista;
        ...
        public async Task<Lista<Caispn09Unidad>> ObtenerTodo(dynamic dynConsulta)
        {
            ...
            var entidad = await uts.AsistenteCaispn09Unidad.ObtenerTodoFiltros();
            if (consulta.Id != null)
            {
            ...
            var entidadPaginada = await lista.Caispn09Unidad.CrearLista(entidad, consulta.NumeroPagina, consulta.NumeroRegistros);
        ...
        public async Task<bool> Actualizar(string matricula)
        {
            ...
            var existe = await uts.AsistenteCaispn09Unidad.ExisteEntidadPorNombre(indicadorNombre);
            if (existe != false)
            {
                var ultimoRegistro = await uts.AsistenteCaispn09Unidad.ObtenerUltimoRegistro(indicadorNombre);
                var pu01 = await uts.AsistenteCaispn09Unidad.ObtenerPeriodosCP04(ultimoRegistro.Periodo);
                var crearIndicador = mapper.Map<List<Caispn09Unidad>>(pu01);
                var indicador = await crear.CrearCAISPN09(crearIndicador, matricula);

                await uts.AsistenteCaispn09Unidad.Eliminar(ultimoRegistro.Id);
                await uts.AsistenteCaispn09Unidad.InsertarLista(indicador);
            }
            else
            {
                var pu01 = await uts.AsistenteCaispn09Unidad.ObtenerPeriodosCP04(Indicadores.Periodo.Inicio);
                var crearIndicador = mapper.Map<List<Caispn09Unidad>>(pu01);
                var indicador = await crear.CrearCAISPN09(crearIndicador, matricula);
                await uts.AsistenteCaispn09Unidad.InsertarLista(indicador);
            }
