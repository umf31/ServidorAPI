...
//               LogicaCaispn14Unidad: Creado 16-06-2022
//=======================================================================
...
{
    public class LogicaCaispn14Unidad<Indicador> : ILogicaCaispn14Unidad<Caispn14Unidad>
    {
        private readonly IMapper mapper;
        private readonly ISadimUT uts;
        private readonly IPagSadim lista;
        private readonly ICrearIndicador crear;
        public LogicaCaispn14Unidad(IMapper _mapper, ISadimUT _uts, IPagSadim _lista, ICrearIndicador _crear)
        {
            mapper = _mapper;
            uts = _uts;
            lista = _lista;
        ...
        public async Task<Lista<Caispn14Unidad>> ObtenerTodo(dynamic dynConsulta)
        {
            ...
            var entidad = await uts.AsistenteCaispn14Unidad.ObtenerTodoFiltros();
            if (consulta.Id != null)
            {
            ...
            var entidadPaginada = await lista.Caispn14Unidad.CrearLista(entidad, consulta.NumeroPagina, consulta.NumeroRegistros);
        ...
        public async Task<bool> Actualizar(string matricula)
        {
            ...
            var existe = await uts.AsistenteCaispn14Unidad.ExisteEntidadPorNombre(indicadorNombre);
            if (existe != false)
            {
                var ultimoRegistro = await uts.AsistenteCaispn14Unidad.ObtenerUltimoRegistro(indicadorNombre);
                var pu01 = await uts.AsistenteCaispn14Unidad.ObtenerPeriodosCP04(ultimoRegistro.Periodo);
                var crearIndicador = mapper.Map<List<Caispn14Unidad>>(pu01);
                var indicador = await crear.CrearCAISPN14(crearIndicador, matricula);

                await uts.AsistenteCaispn14Unidad.Eliminar(ultimoRegistro.Id);
                await uts.AsistenteCaispn14Unidad.InsertarLista(indicador);
            }
            else
            {
                var pu01 = await uts.AsistenteCaispn14Unidad.ObtenerPeriodosCP04(Indicadores.Periodo.Inicio);
                var crearIndicador = mapper.Map<List<Caispn14Unidad>>(pu01);
                var indicador = await crear.CrearCAISPN14(crearIndicador, matricula);
                await uts.AsistenteCaispn14Unidad.InsertarLista(indicador);
            }
