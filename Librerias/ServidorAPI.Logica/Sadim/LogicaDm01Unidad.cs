...
{
    public class LogicaDm01Unidad<Indicador> : ILogicaDm01Unidad<Dm01Unidad>
    {
        private readonly IMapper mapper;
        private readonly ISadimUT uts;
        private readonly IPagSadim lista;
        private readonly ICrearIndicador crear;
        public LogicaDm01Unidad(IMapper _mapper, ISadimUT _uts, IPagSadim _lista, ICrearIndicador _crear)
        {
            mapper = _mapper;
            uts = _uts;
            lista = _lista;
        ...
        public async Task<Lista<Dm01Unidad>> ObtenerTodo(dynamic dynConsulta)
        {
            ...
            var entidad = await uts.AsistenteDm01Unidad.ObtenerTodoFiltros();
            if (consulta.Id != null)
            {
            ...
            var entidadPaginada = await lista.Dm01Unidad.CrearLista(entidad, consulta.NumeroPagina, consulta.NumeroRegistros);
        ...
        public async Task<bool> Actualizar(string matricula)
        {
            ...
            var existe = await uts.AsistenteDm01Unidad.ExisteEntidadPorNombre(indicadorNombre);
            if (existe != false)
            {
                var ultimoRegistro = await uts.AsistenteDm01Unidad.ObtenerUltimoRegistro(indicadorNombre);
                var cp04 = await uts.AsistenteDm01Unidad.ObtenerListaCP04(ultimoRegistro.Periodo);
                var crearIndicador = mapper.Map<List<Dm01Unidad>>(cp04);
                var indicador = await crear.CrearDM01(crearIndicador, matricula);

                await uts.AsistenteDm01Unidad.Eliminar(ultimoRegistro.Id);
                await uts.AsistenteDm01Unidad.InsertarLista(indicador);
            }
            else
            {
                var cp04 = await uts.AsistenteDm01Unidad.ObtenerListaCP04(Indicadores.Periodo.Inicio);
                var crearIndicador = mapper.Map<List<Dm01Unidad>>(cp04);
                var indicador = await crear.CrearDM01(crearIndicador, matricula);
                await uts.AsistenteDm01Unidad.InsertarLista(indicador);
            }
