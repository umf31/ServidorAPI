...
//                LogicaDm05Unidad: Creado 16-06-2022
//=======================================================================
...
{
    public class LogicaDm05Unidad<Indicador> : ILogicaDm05Unidad<Dm05Unidad>
    {
        private readonly IMapper mapper;
        private readonly ISadimUT uts;
        private readonly IPagSadim lista;
        private readonly ICrearIndicador crear;
        public LogicaDm05Unidad(IMapper _mapper, ISadimUT _uts, IPagSadim _lista, ICrearIndicador _crear)
        {
            mapper = _mapper;
            uts = _uts;
            lista = _lista;
        ...
        public async Task<Lista<Dm05Unidad>> ObtenerTodo(dynamic dynConsulta)
        {
            ...
            var entidad = await uts.AsistenteDm05Unidad.ObtenerTodoFiltros();
            if (consulta.Id != null)
            {
            ...
            var entidadPaginada = await lista.Dm05Unidad.CrearLista(entidad, consulta.NumeroPagina, consulta.NumeroRegistros);
        ...
        public async Task<bool> Actualizar(string matricula)
        {
            ...
            var existe = await uts.AsistenteDm05Unidad.ExisteEntidadPorNombre(indicadorNombre);
            if (existe != false)
            {
                var ultimoRegistro = await uts.AsistenteDm05Unidad.ObtenerUltimoRegistro(indicadorNombre);
                var in23 = await uts.AsistenteDm05Unidad.ObtenerIN23(ultimoRegistro.Periodo);
                var crearIndicador = mapper.Map<List<Dm05Unidad>>(in23);
                var indicador = await crear.CrearDM05(crearIndicador, matricula);
                await uts.AsistenteDm05Unidad.Eliminar(ultimoRegistro.Id);
                await uts.GuardarServidorAPI();
                await uts.AsistenteDm05Unidad.InsertarLista(indicador);
            }
            else
            {
                var in23 = await uts.AsistenteDm05Unidad.ObtenerListaIN23(Indicadores.Periodo.Inicio);
                var crearIndicador = mapper.Map<List<Dm05Unidad>>(in23);
                var indicador = await crear.CrearDM05(crearIndicador, matricula);
                await uts.AsistenteDm05Unidad.InsertarLista(indicador);
            }
