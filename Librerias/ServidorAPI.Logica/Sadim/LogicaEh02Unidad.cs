...
//                  LogicaEh02Unidad: Creado 16-06-2022
//=======================================================================
...
{
    public class LogicaEh02Unidad<Indicador> : ILogicaEh02Unidad<Eh02Unidad>
    {
        private readonly IMapper mapper;
        private readonly ISadimUT uts;
        private readonly IPagSadim lista;
        private readonly ICrearIndicador crear;
        public LogicaEh02Unidad(IMapper _mapper, ISadimUT _uts, IPagSadim _lista, ICrearIndicador _crear)
        {
            mapper = _mapper;
            uts = _uts;
            lista = _lista;
        ...
        public async Task<Lista<Eh02Unidad>> ObtenerTodo(dynamic dynConsulta)
        {
            ...
            var entidad = await uts.AsistenteEh02Unidad.ObtenerTodoFiltros();
            if (consulta.Id != null)
            {
            ...
            var entidadPaginada = await lista.Eh02Unidad.CrearLista(entidad, consulta.NumeroPagina, consulta.NumeroRegistros);
        ...
        public async Task<bool> Actualizar(string matricula)
        {
            ...
            var existe = await uts.AsistenteEh02Unidad.ExisteEntidadPorNombre(indicadorNombre);
            if (existe != false)
            {
                var ultimoRegistro = await uts.AsistenteEh02Unidad.ObtenerUltimoRegistro(indicadorNombre);
                var cp04 = await uts.AsistenteEh02Unidad.ObtenerListaCP04_IMCP20(ultimoRegistro.Periodo);
                var crearIndicador = mapper.Map<List<Eh02Unidad>>(cp04);
                var indicador = await crear.CrearEH02(crearIndicador, matricula);
                await uts.AsistenteEh02Unidad.Eliminar(ultimoRegistro.Id);
                await uts.GuardarServidorAPI();
                await uts.AsistenteEh02Unidad.InsertarLista(indicador);
            }
            else
            {
                var cp04 = await uts.AsistenteEh02Unidad.ObtenerListaCP04_IMCP20(Indicadores.Periodo.Inicio);
                var crearIndicador = mapper.Map<List<Eh02Unidad>>(cp04);
                var indicador = await crear.CrearEH02(crearIndicador, matricula);
                await uts.AsistenteEh02Unidad.InsertarLista(indicador);
            }
