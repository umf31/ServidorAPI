...
{
    public class LogicaMaterna02Unidad<Indicador> : ILogicaMaterna02Unidad<Materna02Unidad>
    {
        private readonly IMapper mapper;
        private readonly ISadimUT uts;
        private readonly IPagSadim lista;
        private readonly ICrearIndicador crear;
        public LogicaMaterna02Unidad(IMapper _mapper, ISadimUT _uts, IPagSadim _lista, ICrearIndicador _crear)
        {
            mapper = _mapper;
            uts = _uts;
            lista = _lista;
        ...
        public async Task<Lista<Materna02Unidad>> ObtenerTodo(dynamic dynConsulta)
        {
            ...
            var entidad = await uts.AsistenteMaterna02Unidad.ObtenerTodoFiltros();
            if (consulta.Id != null)
            {
            ...
            var entidadPaginada = await lista.Materna02Unidad.CrearLista(entidad, consulta.NumeroPagina, consulta.NumeroRegistros);
        ...
        public async Task<bool> Actualizar(string matricula)
        {
            ...
            var existe = await uts.AsistenteMaterna02Unidad.ExisteEntidadPorNombre(indicadorNombre);
            if (existe != false)
            {
                var ultimoRegistro = await uts.AsistenteMaterna02Unidad.ObtenerUltimoRegistro(indicadorNombre);
                var mt03 = await uts.AsistenteMaterna02Unidad.ObtenerMT03_Periodos(ultimoRegistro.Periodo);
                var crearIndicador = mapper.Map<List<Materna02Unidad>>(mt03);
                var indicador = await crear.CrearMaterna02(crearIndicador, matricula);

                await uts.AsistenteMaterna02Unidad.Eliminar(ultimoRegistro.Id);
                await uts.AsistenteMaterna02Unidad.InsertarLista(indicador);
            }
            else
            {
                var mt03 = await uts.AsistenteMaterna02Unidad.ObtenerMT03_Periodos(Indicadores.Periodo.Inicio);
                var crearIndicador = mapper.Map<List<Materna02Unidad>>(mt03);
                var indicador = await crear.CrearMaterna02(crearIndicador, matricula);
                await uts.AsistenteMaterna02Unidad.InsertarLista(indicador);
            }
