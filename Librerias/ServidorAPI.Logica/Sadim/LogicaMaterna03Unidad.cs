...
{
    public class LogicaMaterna03Unidad<Indicador> : ILogicaMaterna03Unidad<Materna03Unidad>
    {
        private readonly IMapper mapper;
        private readonly ISadimUT uts;
        private readonly IPagSadim lista;
        private readonly ICrearIndicador crear;
        public LogicaMaterna03Unidad(IMapper _mapper, ISadimUT _uts, IPagSadim _lista, ICrearIndicador _crear)
        {
            mapper = _mapper;
            uts = _uts;
            lista = _lista;
        ...
        public async Task<Lista<Materna03Unidad>> ObtenerTodo(dynamic dynConsulta)
        {
            ...
            var entidad = await uts.AsistenteMaterna03Unidad.ObtenerTodoFiltros();
            if (consulta.Id != null)
            {
            ...
            var entidadPaginada = await lista.Materna03Unidad.CrearLista(entidad, consulta.NumeroPagina, consulta.NumeroRegistros);
        ...
        public async Task<bool> Actualizar(string matricula)
        {
            ...
            var existe = await uts.AsistenteMaterna03Unidad.ExisteEntidadPorNombre(indicadorNombre);
            if (existe != false)
            {
                var ultimoRegistro = await uts.AsistenteMaterna03Unidad.ObtenerUltimoRegistro(indicadorNombre);
                var mt03 = await uts.AsistenteMaterna03Unidad.ObtenerMT03_Periodos(ultimoRegistro.Periodo);
                var crearIndicador = mapper.Map<List<Materna03Unidad>>(mt03);
                var indicador = await crear.CrearMaterna03(crearIndicador, matricula);

                await uts.AsistenteMaterna03Unidad.Eliminar(ultimoRegistro.Id);
                await uts.AsistenteMaterna03Unidad.InsertarLista(indicador);
            }
            else
            {
                var mt03 = await uts.AsistenteMaterna03Unidad.ObtenerMT03_Periodos(Indicadores.Periodo.Inicio);
                var crearIndicador = mapper.Map<List<Materna03Unidad>>(mt03);
                var indicador = await crear.CrearMaterna03(crearIndicador, matricula);
                await uts.AsistenteMaterna03Unidad.InsertarLista(indicador);
            }
