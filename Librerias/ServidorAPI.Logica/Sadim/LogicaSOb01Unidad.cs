...
//                LogicaSOb01Unidad: Creado 16-06-2022
//=======================================================================
...
{
    public class LogicaSOb01Unidad<Indicador> : ILogicaSOb01Unidad<SOb01Unidad>
    {
        private readonly IMapper mapper;
        private readonly ISadimUT uts;
        private readonly IPagSadim lista;
        private readonly ICrearIndicador crear;
        public LogicaSOb01Unidad(IMapper _mapper, ISadimUT _uts, IPagSadim _lista, ICrearIndicador _crear)
        {
            mapper = _mapper;
            uts = _uts;
            lista = _lista;
        ...
        public async Task<Lista<SOb01Unidad>> ObtenerTodo(dynamic dynConsulta)
        {
            ...
            var entidad = await uts.AsistenteSOb01Unidad.ObtenerTodoFiltros();
            if (consulta.Id != null)
            {
            ...
            var entidadPaginada = await lista.SOb01Unidad.CrearLista(entidad, consulta.NumeroPagina, consulta.NumeroRegistros);
        ...
        public async Task<bool> Actualizar(string matricula)
        {
            ...
            var existe = await uts.AsistenteSOb01Unidad.ExisteEntidadPorNombre(indicadorNombre);
            if (existe != false)
            {
                var ultimoRegistro = await uts.AsistenteSOb01Unidad.ObtenerUltimoRegistro(indicadorNombre);
                var cp02 = await uts.AsistenteSOb01Unidad.ObtenerListaCP04(ultimoRegistro.Periodo);
                var crearIndicador = mapper.Map<List<SOb01Unidad>>(cp02);
                var indicador = await crear.CrearSOb01(crearIndicador, matricula);

                await uts.AsistenteSOb01Unidad.Eliminar(ultimoRegistro.Id);
                await uts.AsistenteSOb01Unidad.InsertarLista(indicador);
            }
            else
            {
                var cp02 = await uts.AsistenteSOb01Unidad.ObtenerListaCP04(Indicadores.Periodo.Inicio);
                var crearIndicador = mapper.Map<List<SOb01Unidad>>(cp02);
                var indicador = await crear.CrearSOb01(crearIndicador, matricula);
                await uts.AsistenteSOb01Unidad.InsertarLista(indicador);
            }
