...
//                LogicaStatus: Creado 14-06-2022
//=======================================================================
...
{
    public class LogicaStatus<Entidad> : ILogicaStatus<Status>
    {
        ...
        public LogicaStatus(IMapper _mapper, IServidorUT _ut, IPagServidor _lista, IUtilerias _util)
        {
            mapper = _mapper;
            ut = _ut;
            lista = _lista;
        ...
        public async Task<Lista<Status>> ObtenerTodo(dynamic dynConsulta)
        {
            var consulta = (StatusConsulta)dynConsulta;
            ...
            var entidad = await ut.AsistenteStatus.ObtenerTodo();
            if (consulta.Id != null)
            {
            ...
            var entidadPaginada = await lista.Status.CrearLista(entidad, consulta.NumeroPagina, consulta.NumeroRegistros);
        ...
        public async Task<Status> ObtenerPorId(int entidadId)
        {
            var entidad = await ut.AsistenteStatus.ObtenerPorId(entidadId);
            if (entidad == null)
            {
                throw new NotFound(Mensaje.Detalle.NoEncontrado);
            }
        ...
        public async Task<Status> Insertar(dynamic dynEntidadInsertar, string matricula)
        {
            var entidadInsertar = (StatusInsertar)dynEntidadInsertar;
            ...
            if (await ut.AsistenteStatus.ExisteEntidadPorNombre(entidadInsertar.Nombre))
            {
                throw new BadRequest(Mensaje.Detalle.SolicitudInvalida);
            }
            var entidad = mapper.Map<Status>(entidadInsertar);
            ...
            await ut.AsistenteStatus.Insertar(entidad);
        ...
        {
            await ut.AsistenteStatus.Actualizar(entidadId, matricula);
            await ut.GuardarServidorAPI();
            return true;
        }
        ...
        {
            var entidadEditar = (StatusEditar)dynEntidadEditar;
            ...
            if (!await ut.AsistenteStatus.ExisteEntidadPorId(id))
            {
                throw new NotFound(Mensaje.Detalle.NoEncontrado);
            }
            if (await ut.AsistenteStatus.ExisteEntidadPorNombre(entidadEditar.Nombre))
            {
                throw new BadRequest(Mensaje.Detalle.SolicitudInvalida);
            }
            var entidad = await ut.AsistenteStatus.ObtenerPorId(id);
            ...
            await ut.AsistenteStatus.Editar(entidad);
            await ut.GuardarServidorAPI();
            return true;
        }
        ...
        public async Task<bool> Eliminar(int entidadId, string matricula)
        {
            ...
            if (empleadoRol != null)
            {
                await ut.AsistenteStatus.Eliminar(entidadId);
            }
            else
            {
                await ut.AsistenteStatus.Actualizar(entidadId, matricula);
