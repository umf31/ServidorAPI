...
//            AsistenteMaterna02Unidad: Creado 15-06-2022
//=======================================================================
...
{
    public class AsistenteMaterna02Unidad : AsistenteIndicador<Materna02Unidad>, IAsistenteMaterna02Unidad
    {
        private readonly SadimContexto dbs;
        public AsistenteMaterna02Unidad(SadimContexto _dbs, ServidorContexto db) : base(db, _dbs)
        {
            dbs = _dbs;
        }

        public async Task<IEnumerable<Materna02Unidad>> ObtenerTodoFiltros()
        {
            var entidad = indicador.Where(x => x.StatusId == 1).Include(c => c.Periodos).AsEnumerable();
            return await Task.FromResult(entidad);
        }
        public async Task<MT03_VigilanciaMaterna> ObtenerMT03_OportunidadVigilanciaPrenatal(string periodo)
        {
            var mt03_vigmat = await dbs.MT03_VigilanciaMaterna
                .Where(x => x.Gesta == 0 && x.Vez == "1" && x.Concepto == "1" && x.Periodo == periodo).FirstOrDefaultAsync();
            if (mt03_vigmat != null)
            {
                return mt03_vigmat;
            }
        ...
        {
            return await dbs.MT03_VigilanciaMaterna.Where(x => x.Gesta == 0 && x.Vez == "1" && x.Concepto == "0" && Convert.ToInt32(x.Periodo) >= Convert.ToInt32(periodo)).OrderBy(x => x.Periodo).ToListAsync();
        }
    }
}
